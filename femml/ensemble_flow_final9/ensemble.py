...
def ensemble(f_list, J, mesh, mesh_size, t_init, TIME_STEP, T, nu):
    """
    ...
    for i in range(J):
        np.savetxt('./output/condition/ensemble' + str(i) + '.txt', condition[i])
        plt.figure(0)
        plt.plot(x, condition[i])
        plt.xlabel('t')
        plt.xlim((t_init, T))
        plt.ylabel('condition number')
        plt.savefig('./fig/condition/ensemble' + str(i) + '.png')
        plt.close('all')
    for i in range(J):
...
def ensemble_penalty(f_list, J, mesh, mesh_size, TIME_STEP, t_init, T, nu, mu, beta, epsilon):
    """
    Solves the incompressible Navier Stokes equations
    the FEniCS Python Package
    Momentum:           ∂u/∂t -ν ∇²u + (u ⋅ ∇) u + ∇p = f
    ...
    x = np.linspace(t_init, T, int((T - t_init) / TIME_STEP) + 1)
    for i in range(J):
        np.savetxt('./output/condition/ensemble_penalty' + str(i) + '.txt', condition[i])
        plt.figure(0)
        plt.plot(x, condition[i])
        plt.xlabel('t')
        plt.xlim((t_init, T))
        plt.ylabel('condition number')
        plt.savefig('./fig/condition/ensemble_penalty' + str(i) + '.png')
        plt.close('all')
    for i in range(J):
