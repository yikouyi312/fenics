...
def ensemble_EVV_penalty(f_list, J, mesh, SPACE_STEP, TIME_STEP, t_init, T, nu, mu, beta, epsilon, type1 = 2, type2 = 2):
    """
    ...
    while count_time < N - 1:
        ...
        for i in range(J):
            ...
            # print(str(i) + "th member: ")
            # print(max(project(cal_speed(u_next), Q).vector()))
            ...
            if count_time % frameRat == 0:
                ...
                if max(u_next.vector()) > 1e-2:
                    print(str(i) + "th member: ")
                    print(max(u_next.vector()))
        ...
        if count_time % frameRat == 0:
            ...
            plt.close('all')
            if max(project(u_avg, V).vector()) > 1e-2:
    ...
    for i in range(J):
        np.savetxt('./output/condition/ensemble_penalty_EVV_evvtype' + str(type1) + '_evv1type' + str(type2) + str(i) +'.txt', condition[i])
        plt.figure(0)
        plt.plot(x, condition[i])
        plt.xlabel('t')
        plt.xlim((t_init, T))
        plt.ylabel('condition number')
        plt.savefig('./fig/condition/ensemble_penalty_EVV_evvtype' + str(type1) + '_evv1type' + str(type2) + str(i) + '.png')
        plt.close('all')
    for i in range(J):
