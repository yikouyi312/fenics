...
def ensemble_EVV(u_list, p_list, f_list, u_exact_avg, J, mesh, SPACE_STEP, TIME_STEP, T, nu, mu, type = 2):
    """
    ...
    while count_time < N - 1:
        ...
        u_exact_avg.s = t
        u_avg = interpolate(u_exact_avg, V)
        bcs.append(DirichletBC(W.sub(0), u_avg, boundary))
        # cur = time.time()
        A = assemble(A_u)
        ...
        for i in range(J):
            ...
            w_next = Function(W)
            solver.solve(w_next.vector(), B) # solve(A, w_next.vector(), B)
